...
class LigandKinematics(nn.Module):
    def __init__(self, mol: Chem.Mol, ref_indices: List[int], init_coords: torch.Tensor, device: torch.device, freeze_mcs: bool = True):
        ...
        # 1. Identify rotatable bonds (using SMARTS)
        rot_smarts = Chem.MolFromSmarts('[!$(*#*)&!D1]-&!@[!$(*#*)&!D1]')
        rot_bonds = mol.GetSubstructMatches(rot_smarts)

        ref_set = set(ref_indices)

        # Filter rotatable bonds: if freeze_mcs and BOTH atoms are in the MCS, freeze the bond (treat as rigid)
        rot_bond_pairs = []
        if freeze_mcs:
        ...
        # 2. Identify rigid frames by breaking rotatable bonds
        # We'll do a simple BFS graph traversal
        adj_list = {i: [] for i in range(self.num_atoms)}
        ...
        # 3. Build a directed tree starting from the Root Frame (which contains the largest intersection with ref_indices)
        root_frame_idx = -1
        max_overlap = -1
        ...
        # Tree structure: frame -> List of (child_frame, parent_atom, child_atom)
        self.tree = {i: [] for i in range(len(frames))}
        # Rotatable bonds function as directed edges
        # Start BFS over frames from root
        q = [root_frame_idx]
        visited_frames = {root_frame_idx}

        self.kinematic_edges = [] # List of (parent_atom, child_atom, child_frame_idx)

        while q:
            curr_frame = q.pop(0)

            # Find any rotatable bond connecting curr_frame to an unvisited frame
            for u in frames[curr_frame]:
                # Look at mol neighbors of u
                atom = mol.GetAtomWithIdx(u)
                for neighbor in atom.GetNeighbors():
                    v = neighbor.GetIdx()
                    neighbor_frame = atom_to_frame[v]
                    if neighbor_frame != curr_frame and neighbor_frame not in visited_frames:
                        # Found a rotatable bond leading to a new frame
                        # u is parent, v is child
                        self.tree[curr_frame].append((neighbor_frame, u, v))
                        self.kinematic_edges.append((u, v, neighbor_frame))
                        visited_frames.add(neighbor_frame)
                        q.append(neighbor_frame)

        self.num_torsions = len(self.kinematic_edges)
        self.frames = frames
        ...
        self.parent_atoms = [edge[0] for edge in self.kinematic_edges]
        self.child_atoms = [edge[1] for edge in self.kinematic_edges]
        self.child_frames = [edge[2] for edge in self.kinematic_edges]

        # OPTIMIZED: Precompute atoms_to_rotate and frame_descendants
        self.frame_descendants_cache = {}
        for frame_idx in range(len(frames)):
            self.frame_descendants_cache[frame_idx] = self._compute_descendants_once(frame_idx)

        self.atoms_to_rotate_tensors = []
        for i in range(self.num_torsions):
            frame_child = self.child_frames[i]
            atoms_to_rotate = self.frames[frame_child] + self.frame_descendants_cache[frame_child]
            self.atoms_to_rotate_tensors.append(
                torch.tensor(atoms_to_rotate, dtype=torch.long, device=self.device)
            )

    def _compute_descendants_once(self, frame_idx: int) -> List[int]:
        """Compute descendants recursively - called once during __init__."""
        descendants = []
        for child_frame, _, _ in self.tree[frame_idx]:
            descendants.extend(self.frames[child_frame])
            descendants.extend(self._compute_descendants_once(child_frame))
        return descendants
