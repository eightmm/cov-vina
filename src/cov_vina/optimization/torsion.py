...
def optimize_torsions_vina(mol: Chem.Mol,
    ...
    Args:
        mol: RDKit molecule
        ref_indices: Anchor atom indices to freeze
        ...
        freeze_anchor: Whether to freeze anchor atoms (default: True)
    ...
    # Expand exclusion masks for batch if needed
    if intermolecular_exclusion_mask is not None and intermolecular_exclusion_mask.shape[0] == 1:
        # Will expand to batch size dynamically during optimization
        pass

    # Check if molecule has rotatable bonds
    test_model = LigandKinematics(mol, ref_indices, init_coords[0], device, freeze_anchor=freeze_anchor)
    ...
    for batch_idx in range(n_batches):
        ...
        if optimizer.lower() == 'lbfgs':
            ...
            for i in range(start_idx, end_idx):
                model = LigandKinematics(mol, ref_indices, init_coords[i], device, freeze_anchor=freeze_anchor)
            ...
            for step in range(num_steps):
                ...
                for i, (model, opt) in enumerate(zip(models, optimizers_list)):
                    ...
                    def closure():
                        opt.zero_grad()
                        coords = model()
                        # Expand masks for single pose
                        single_intra_mask = intra_mask.unsqueeze(0)
                        single_intermol_mask = None
                        if intermolecular_exclusion_mask is not None:
                            single_intermol_mask = intermolecular_exclusion_mask  # Already has batch dim=1
            ...
            for step in range(num_steps):
                ...
                batch_intra_mask = intra_mask.unsqueeze(0).expand(batch_len, -1, -1)
                batch_intermol_mask = None
                if intermolecular_exclusion_mask is not None:
                    batch_intermol_mask = intermolecular_exclusion_mask.expand(batch_len, -1, -1)
