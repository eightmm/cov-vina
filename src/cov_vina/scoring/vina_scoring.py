...
def vina_scoring(aligned_query_coords: torch.Tensor,
                 ...
                 intermolecular_exclusion_mask: torch.Tensor = None) -> torch.Tensor:
    """
    ...
    Args:
        intermolecular_exclusion_mask: Boolean tensor (batch_size, num_query_atoms, num_pocket_atoms)
                                       True = exclude this pair from intermolecular scoring
                                       Used for covalent docking to exclude covalent bond region
    """
    ...
    # Apply intermolecular exclusion mask for covalent docking
    if intermolecular_exclusion_mask is not None:
        # Zero out excluded interactions
        energy_matrix = energy_matrix * (~intermolecular_exclusion_mask).float()
    ...
    if intramolecular_mask is not None:
        ...
        # Apply mask and sum
        # Expand intramolecular_mask to match batch size
        if intramolecular_mask.dim() == 2:
            # (N, N) → (1, N, N) → (batch_size, N, N)
            intra_mask_expanded = intramolecular_mask.unsqueeze(0).expand(batch_size, -1, -1)
        elif intramolecular_mask.dim() == 3:
            # Already (batch_size, N, N) or (1, N, N)
            if intramolecular_mask.shape[0] == 1 and batch_size > 1:
                intra_mask_expanded = intramolecular_mask.expand(batch_size, -1, -1)
            else:
                intra_mask_expanded = intramolecular_mask
        else:
            intra_mask_expanded = intramolecular_mask

        masked_intra = intra_energy_matrix * intra_mask_expanded.float()
